...
export async function guardarAfiliadoAction(formData: any, idEditar?: string) {
  ...
  if (formData.empadronado && formData.no_padron) {
    let query = supabase
      .from("afiliados")
      .select("id")
      .eq("no_patron", formData.no_padron);
    if (idEditar) query = query.neq("id", idEditar);
  ...
  // AQUI EL CAMBIO: politica ya no depende de empadronado
  const dataToSend = {
    ...formData,
    no_patron: formData.empadronado ? formData.no_padron : null,
    politica: formData.politica || null, // Guarda lo que venga
  };

  delete dataToSend.no_padron;
