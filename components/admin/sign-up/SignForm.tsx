...
export function SignupForm({ onSuccess, isModal = false }: SignupFormProps) {
  ...
  useEffect(() => {
    if (error) {
      ...
      // No se realiza ninguna navegación. El modal queda abierto para corregir.
    }
    ...
    if (success) {
      Swal.fire({
        ...
        // En caso de éxito, solo llamamos al callback si es un modal.
        // Esto permite al componente padre (Ver.tsx) cerrar el modal y refrescar.
        if (isModal) {
            onSuccess();
        } 
        // IMPORTANTE: Eliminada la navegación (`router.push`) para mantener el foco en la acción del modal.
      });
    }
  }, [error, success, isModal, onSuccess]);
  ...
  const handleNumericChange = (e: React.ChangeEvent<HTMLInputElement>, setter: React.Dispatch<React.SetStateAction<string>>, maxLength: number) => {
    const value = e.target.value.replace(/\D/g, 'g'); 
  ...
  return (
    <div className="flex flex-col w-full mx-auto md:max-w-xl gap-6">
      ...
      <span className="text-blue-600 font-extrabold md:text-xl">Ingresa los datos del nuevo Líder</span>
